      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-slate-900">Cadastro de Clientes</h2>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {clients.map(cli => (
              <div
                key={cli.id}
                className="p-4 bg-white border border-slate-200 rounded-2xl flex flex-col justify-between hover:border-slate-300 transition-all shadow-xs"
              >
                <div>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <h4 className="font-bold text-slate-900 text-sm truncate">{cli.name}</h4>
                    {cli.cpf && (
                      <span className="px-2 py-0.5 rounded-md text-[10px] font-mono bg-slate-100 text-slate-600 shrink-0 border border-slate-200">
                        {cli.cpf}
                      </span>
                    )}
                  </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
                  >
          ...
          <div className="bg-white border border-slate-200 rounded-2xl max-w-md w-full shadow-2xl overflow-hidden my-auto">
            ...
            <form onSubmit={handleSaveClient} className="p-6 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-600 font-semibold mb-1">CPF (Opcional)</label>
                  <input
                    type="text"
                    value={cpf}
              ...
              <div>
                <label className="block text-slate-600 font-semibold mb-1">Endereço</label>
                <input
                  type="text"
                  value={address}
              ...
              <div>
                <label className="block text-slate-600 font-semibold mb-1">Observações Internas</label>
                <textarea
                  rows={2}
                  value={notes}
